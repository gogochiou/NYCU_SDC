...
save_dir = Path(config['save_model']['path'])
...
if __name__ == '__main__':
    if args.train:
        ...
        checkpoint_callback = ModelCheckpoint(
            dirpath=ckpt_dir, 
            filename='{epoch:02d}-{train_loss:.2f}_batch48',
        ...
        print(model) # check model structure
        trainer.fit(model)
        # x = torch.rand(1,300)
        # y = model(x)
        # torch.onnx.export(model, x, "./models/model.pth")

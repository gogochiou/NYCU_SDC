...
private:
  ...
  float pass_scan_map[3]={10.0, 10.0, 10.0}, pass_map_range[3]={50.0, 50.0, 1.0};

  ros::NodeHandle _nh;
  ros::Subscriber sub_map, sub_points, sub_gps, sub_imu, sub_odom;
...
public:
  ...
  Eigen::Matrix4f align_map(const pcl::PointCloud<pcl::PointXYZI>::Ptr scan_points, const sensor_msgs::PointCloud2::ConstPtr& msg){
    ...
    downsampling_filter("voxel", voxel_scan_range, scan_points, filtered_scan_ptr);
    ...
    // downsampling_filter("passthrough_xy", pass_map_range, map_points, filtered_map_ptr);
    downsampling_filter("nofilter", pass_map_range, map_points, filtered_map_ptr);

...
public:
  Localizer(ros::NodeHandle nh): map_points(new pcl::PointCloud<pcl::PointXYZI>) {
    ...
    sub_imu = _nh.subscribe("/imu/data", 400, &Localizer::imu_callback, this);
  ...
  void pc_callback(const sensor_msgs::PointCloud2::ConstPtr& msg){
    ...
    transform_m2l.matrix() = result.cast<double>(); // map 2 lidar
    transform_c2l = (tf2::transformToEigen(car2Lidar)); // car to lidar
  ...
  Eigen::Matrix4f align_map(const pcl::PointCloud<pcl::PointXYZI>::Ptr scan_points){
    ...
    if(!initialied){
      ...
      for(yaw = 2.0; yaw <= 3.0; yaw += 0.01){ //2*M_PI

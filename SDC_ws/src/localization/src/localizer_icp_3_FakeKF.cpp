...
// using odom predict, than gps for update
// set icp change cov is not better
...
public:
  ...
  Eigen::Matrix4f align_map(const pcl::PointCloud<pcl::PointXYZI>::Ptr scan_points, const sensor_msgs::PointCloud2::ConstPtr& msg){
    ...
    // float ratio = state_cov(0, 0)+state_cov(1, 1);
    // state_cov(0, 0) = (float)score + (score*state_cov(0, 0)/ratio);
    // state_cov(1, 1) = (float)score + (score*state_cov(1, 1)/ratio);
    // state_cov(0, 0) += (float)score;
    // state_cov(1, 1) += (float)score;
    // state_cov(0, 0) = (float)score*10;
    // state_cov(1, 1) = (float)score*10;

...
class Fusion:
    ...
    def odometryCallback(self, data):
        ...
        ## Method 2
        # odometry_covariance = np.identity(3)
        # row, col = np.diag_indices_from(odometry_covariance)
        # temp = np.array(data.pose.covariance).reshape(6, -1)[:3,:3]/self.last_diff_cov
        # row2, col2 = np.diag_indices_from(temp)
        # odometry_covariance[row,col] = temp[row2, col2]
        # odometry_covariance[2,2] = 0
        # print(f"Radar cov : \r\n{odometry_covariance}\r\n") 

        # print(f"Radar cov : {odometry_covariance}") 
        ...
        print(f"estimation_predict: {self.KF.x}")
        # print(f"P_predict: {self.KF.P}")
        self.last_odometry_position = position
        self.last_odometry_angle = yaw
        self.last_diff_cov = np.array(data.pose.covariance).reshape(6, -1)[:3,:3]

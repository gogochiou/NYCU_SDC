...
def argo_multi_agent_collate_fn(batch):
    # print("Inside argo_multi_agent_collate_fn")
    elem = [k for k, v in batch[0].items()]
    def _collate_util(input_list, key):
        if isinstance(input_list[0], torch.Tensor):
            return torch.cat(input_list, 0)
...
def plot_argo_scenario(sample):
    ...
    print(lane.size())
    for i in range(lane.size(0)):
        plt.scatter(lane[i][:,0], lane[i][:, 1], s=0.5, c='w') # s - size(scaler)

    ''' Lane border'''
    lane_b = sample['lane_polygon']
    # print(lane_b)
    for i in range(lane_b.size(0)):
        plt.plot(lane_b[i][:,0], lane_b[i][:,1], 'w')

    ''' Cross walk'''
    crosswalk = sample['crosswalk_polygon'] # polygon type
    # crosswalk = sample['crosswalk_graph'] # border type
    for i in range(crosswalk.size(0)):
        plt.plot(crosswalk[i][:,0], crosswalk[i][:,1], 'y')
    
    '''Surrounding object motion'''
    neighbor_path = sample['neighbor_graph']
    # print(neighbor_path.size())
    for i in range(neighbor_path.size(0)) :
        plt.plot(neighbor_path[i][:,0], neighbor_path[i][:,1],'b', linewidth=2.0)
